...
<form action="index.php?act=update-product" method="POST" enctype="multipart/form-data">
    ...
    <div class="mb-3">
        ...
        <input type="text" class="form-control" name="name" placeholder="Nhập vào tên" value="<?php echo $product['name'] ?>">
    </div>
    <div class="mb-3">
        <label for="" class="form-label">Giá SP</label>
        <input type="number" class="form-control" name="price" placeholder="Nhập vào giá" value="<?php echo $product['price'] ?>">
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <textarea class="form-control" name="description" rows="3"><?php echo $product['name'] ?></textarea>
    </div>
    <div class="mb-3">
        ...
        <select name="id-category" id="" class="form-control" style="padding: 4px;">
            ...
            foreach ($listCategory as $value) {
                extract($value);
                ?>
                <option <?php echo $product['iddm'] == $id ? 'selected' : '' ?> value='<?php echo $id ?>'><?php echo $name ?></option>
                <?php
            }
            ?>
        </select>
