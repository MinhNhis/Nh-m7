...
class Product
{
    ...
    function updatePProduct($name, $price, $fileImg, $description, $idCategory, $id)
    {
        if ($fileImg['name']) {
            $nameImg = $fileImg['name'];
            $tmpImg = $fileImg['tmp_name'];
            move_uploaded_file($tmpImg, '../upload/' . $nameImg);
            $sql = "UPDATE SANPHAM SET  NAME='$name', PRICE=$price, IMG='$nameImg', MOTA='$description', IDDM='$idCategory' WHERE ID=$id";
            pdo_execute($sql);
        } else {
            $sql = "UPDATE SANPHAM SET  NAME='$name', PRICE=$price, MOTA='$description', IDDM='$idCategory' WHERE ID=$id";
            pdo_execute($sql);
        }
    }
